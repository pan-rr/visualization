 ...
 */
const handleUpload = (file, fileChunkRecord, options) => {

    let lastUploadedSize = 0; // 上次断点续传时上传的总大小
    let uploadedSize = 0 // 已上传的大小
    const totalSize = file.size || 0 // 文件总大小
    let startMs = new Date().getTime(); // 开始上传的时间
    const { exitPartList, chunkSize, chunkNum, md5 } = fileChunkRecord
...
const handleHttpRequest = async (options) => {
    ...
    if (task) {
        ...
        if (finished) {
            ...
            const errorList = await handleUpload(file, fileChunkRecord, options)
